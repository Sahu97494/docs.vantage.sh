...
description: "Vantage has a few options to connect to AWS, including CloudFormation, the AWS CLI, a Terraform module, and the AWS IAM Console."
---
...
- **Terraform:** Click the **More Connection Options** dropdown and select **Connect via AWS Terraform**. Instructions for how to connect using the Terraform module are provided on this screen. See the [section below](/connecting_aws#connect-with-terraform) for more details.
- **AWS IAM Console:** Click the **More Connection Options** dropdown and select **Connect via AWS IAM Console**. Instructions are provided on this screen. See the [section below](/connecting_aws#connect-by-manually-creating-an-iam-role) for more details.
...
During onboarding, instead of following the CloudFormation process, you can use the Vantage Terraform module. See the [Terraform documentation](/terraform#vantage-terraform-integrations-module-for-aws) for more information.
...
If you decide to use this integration method, ensure you’ve set up a Cost and Usage Report (CUR) in your account so that Vantage will be able to pull in and read data from the CUR. After adding the IAM role ARN to Vantage, contact [Vantage Support](/) with your CUR configuration.
</Info>

<Steps>
    <Step>
    On the AWS connection page in the Vantage console, click **More Connection Options** > **Connect via AWS IAM Console**.
    </Step>
    <Step>
    You will be presented with a **Trust Relationship** and an **Inline Policy** that is required for the role. Follow the steps provided in the Vantage console to set up the role. Keep this tab open as you will need some submit the **[ARN](https://docs.aws.amazon.com/IAM/latest/UserGuide/reference-arns.html)** of the role to complete the connection.
    </Step>
    <Step>
    After creating the role in IAM, return to the onboarding page in Vantage and add the **ARN** of the created role to the **Cross account arn** field at the bottom of the connection workflow pop-up window. Then, click **Connect** to complete the connection.
    </Step>
</Steps>
...
- [Cost Reports](/cost_reports) will show line-item data per service initially but will not be able to show *costs by resource* until the first CUR is received.
...
These files should be a daily CSV with resource ids enabled
</Info>
...
<Steps>
    <Step>
    From your AWS root/management account, use the top search bar to search for and navigate to **Data Exports**.
    </Step>
    <Step>
    A table of data exports is displayed. Observe the **Export Name** column. You should see the legacy CUR export that was created by Vantage when the integration was set up. Take note of the values in the **Export Name** and **S3 Bucket** columns.
    </Step>
    <Step>
        Open a ticket with [AWS Support](https://docs.aws.amazon.com/cur/latest/userguide/billing-get-answers.html) to backfill the Vantage export. You can use the sample email template below when creating your request.

        <Accordion title="AWS Support Request Template">
    ...
    <Step>
    Once AWS Support completes your request, please email [support@vantage.sh](mailto:support@vantage.sh) to re-ingest the backfilled legacy CUR Data Export.
    </Step>
</Steps>
...
<Steps>
    <Step>
    Navigate to the [**Integrations** page](https://console.vantage.sh/settings/integrations) of the Vantage console.
    </Step>
    <Step>
    From the **AWS** tile, click **Manage**.
    </Step>
    <Step>
    The **Account** list is displayed. Click **Manage** for the cloud account that you want to update. (For AWS organizations with multiple accounts, this will usually be the Management account.)
    </Step>
    <Step>
    Click **Update Vantage via AWS Console** to perform the update, or click **More Update Options** for command-line and other deployment methods.
    </Step>
</Steps>
...
You can import Amazon CloudWatch metrics as business metrics to view alongside Cost Reports. See the [Per Unit Costs](/per_unit_costs#import-from-amazon-cloudwatch) documentation for more information.
