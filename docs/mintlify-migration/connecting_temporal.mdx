...
The Finance Admin role has the same permissions as Account Read-only users, with additional permissions to view billing information and update payment information. Vantage will never perform any actions to change your payment method. It's recommended that you submit a feature request to Temporal Cloud for improved permission scoping, and Vantage will adopt fine-grained permissions if this becomes available.
</Info>
...
<Steps>
    <Step>
    In Temporal Cloud, navigate to **[Settings > Identities](https://cloud.temporal.io/settings/identities)**.
    </Step>
    <Step>
    At the top, click **Create Service Account**.
    <Accordion title="Click to view example image">
        <Frame>
        ![Creating a Temporal Service Account](https://assets.vantage.sh/docs/temporal-create-service-account.png)
        </Frame>
    </Accordion>
    </Step>
    <Step>
    Enter an **Identity Name**, such as *vantage\_integration*.
    </Step>
    <Step>
    Add an optional description to identify this account.
    </Step>
    <Step>
    For **Account Level Role**, select **Finance Admin**.
    </Step>
    <Step>
    For **Namespace Permissions**, set the access for each namespace to **Read-Only**.
    <Accordion title="Click to view example image">
        <Frame>
        ![Setting up the Temporal service account with relevant permissions](https://assets.vantage.sh/docs/temporal-service-account.png)
        </Frame>
    </Accordion>
    <Info>
    **NOTE**
    ...
    <Step>
    Click **Create Service Account**. Once created, you will be prompted to create an API key for the service account.
    </Step>
</Steps>
...
<Steps>
    <Step>
    After you create the service account, you will be prompted to create an API key. You can also create an API key for the service account at any time. Navigate to **[Settings > API keys](https://cloud.temporal.io/settings/api-keys)**.
    </Step>
    <Step>
    Click **Create API Key**.
    <Accordion title="Click to view example image">
        <Frame>
        ![Creating a new Temporal API key](https://assets.vantage.sh/docs/temporal-create-api.png)
        </Frame>
    </Accordion>
    </Step>
    <Step>
    Under **Create an API key for**, select **Service Account**.
    </Step>
    <Step>
    For **Mapped to Identity**, search for and select the service account you just created.
    </Step>
    <Step>
    Give the API key a **Name** and optional **Description**.
    </Step>
    <Step>
    For **Expiry Date**, select a date for when the key should expire.
    <Info>
    **NOTE**
    ...
    <Accordion title="Click to view example image">
        <Frame>
        ![Creating a new Temporal API key with relevant permissions](https://assets.vantage.sh/docs/temporal-create-api-key.png)
        </Frame>
    </Accordion>
    </Step>
    <Step>
    Click **Generate API Key** and copy the newly generated key.
    </Step>
</Steps>

#### Step 3 - Add Your API Key and Pricing Details to Vantage
...
<Steps>
    ...
    <Step>
    On the left navigation, select **Integrations** > **Temporal Cloud**.
    </Step>
    <Step>
    The Temporal Cloud integrations page is displayed. Ensure you are on the **Connect** tab.
    </Step>
    <Step>
    At the bottom of the page, click **Add API Key** and paste your newly generated API key. Optionally, add a **Description** to identify this integration.
    </Step>
    <Step>
    If you use over 5M Actions or have custom rates, you can enter your blended rates on the form. By default, these fields are pre-filled with Temporal Cloud's public pricing. If you don't have a private rate card or use under 5M Actions, you can leave these fields as-is. See the [How Temporal Cloud Costs Are Calculated](/connecting_temporal#how-temporal-cloud-costs-are-calculated) section for details.
    - Price Per Action
    ...
    **NOTE**
    ...
    <Accordion title="Click to view example image">
        <Frame>
        ![Adding all credentials and custom rates in the Vantage console](https://assets.vantage.sh/docs/temporal-vantage-console.png)
        </Frame>
    </Accordion>
    </Step>
    <Step>
...
<Steps>
    <Step>
    Navigate to the Temporal Cloud [settings page](https://console.vantage.sh/settings/temporal).
    </Step>
    <Step>
    Select your integration from the list.
    </Step>
    <Step>
    Scroll down to the **Custom Rates** section.
    </Step>
    <Step>
    Update your rates accordingly, then click **Save**.
    <Accordion title="Click to view example image">
        <Frame>
        ![Editing rates for the Temporal Cloud integration in Vantage](https://assets.vantage.sh/docs/temporal-edit-rates.png)
        </Frame>
    </Accordion>
    </Step>
</Steps>
