---
title: "Vantage Changelog"
description: "Product updates and improvements"
rss: true
---

<Tip>
This page was last updated on August 15, 2025, with product updates for August 2025.
</Tip>
...
- **Filter UI update:** An update was made to the **where** section of a [report filter](/cost_reports#filter-cost-reports) for improved dropdown usability by auto-focusing the search input when selecting from large filterable lists.
- **Show/hide previous period:** You can now show or hide the [previous period](/cost_reports#create-a-cost-report) on Cost Reports. A new toggle is displayed on the report date picker.
- **Active Resources and Resource Report updates:** The following updates were made to update Resource Reports and [Active Resources](/active_resources).
	...
	- You can now [customize](/active_resources#customize-a-resource-report) which columns are displayed in your Resource Reports, such as resource-specific metadata fields, as well as arrange the columns in any order.
- **MSP RBAC enhancements:** Two enhancements [were released](/partners#role-based-access-control) for Managed Service Provider (MSP) access controls:
...
- The agent [now collects](/kubernetes#filter-by-pvc-and-node-labels) Node and PVC labels for additional reporting metrics.
- With [Persistent Metrics Recovery](/kubernetes_agent#persistent-metrics-recovery), resource metrics are retained when the upload endpoint is unreachable.
...
- **Billing rules parameter:** The `sql_query` parameter is available on the `/billing_rules` [endpoint](https://vantage.readme.io/reference/createbillingrule) and supports creating [custom billing rules](/partners#custom-billing-rules) for MSP customers.
...
- **Hourly granularity for Financial Commitment Reports:** You can now view Financial Commitment Reports at an [hourly granularity](/financial_commitment_reports#hourly-granularity).
- **SQL Billing Engine**: MSP customers now can create billing rules via the [SQL Billing Engine](/partners#custom-billing-rules), allowing for any custom logic on billing data on both Vantage-schema data and AWS Cost and Usage data.
...
*See [May's update](/changelog#kubernetes-agent-updates-3) for the most recent Kubernetes agent release.*
...
- **FOCUS-compatible exports**: You can now [export](/cost_reports#export-cost-reports) your cloud cost and usage data in the FinOps Foundation’s open-source FinOps Open Cost and Usage Specification ([FOCUS](https://focus.finops.org/)) format.
...
- **Previous Period Usage Metrics**: When you view a [Usage-Based Cost Report](/usage_based_reporting/), the table view will now show previous period usage in addition to previous period cost.
...
**Configurable polling period:** You can now set [configurable polling periods](/kubernetes_agent#configure-polling-interval) for the Vantage Kubernetes agent, as short as 5 seconds.
...
- **Datadog query syntax:** You can now use [Datadog queries](/per_unit_costs#import-from-datadog) directly within Vantage to import Business Metrics.
- **Unit Cost export:** You can now [export Unit Costs](/cost_reports#export-cost-reports) from Cost Reports from the Vantage console.
...
- **Vantage for MSPs updates:** The following updates are available for [Vantage for MSPs](/partners) functionality:
	- A new adjustment for [Re-Rate to Public Pricing](/partners#re-rate-to-public-pricing-adjustment) allows Vantage for MSPs users to revert their customers’ pricing back to list price, removing any enterprise discounts or private pricing
	- A new adjustment for [Remove SP/RI Discounts](/partners#remove-sp%2Fri-discounts-adjustment) lets Vantage for MSPs users decide which Reserved Instance and Savings Plan benefits their end customers receive.
	- A new adjustment for [Re-Rate Tiered Discounts](/partners#re-rate-tiered-discounts) lets Management Accounts recalculate tiered pricing models for applicable services for a customer's Managed Account.
...
- **Kubernetes report updates:** The following updates are available for Kubernetes Cost Reports and Efficiency Reports:
	- The Kubernetes `__idle__` namespace is now enabled by default on [Kubernetes Efficiency Reports](/kubernetes#understanding-the-idle-namespace-in-kubernetes-efficiency-reports).
...
The Vantage Kubernetes agent now supports ingesting costs for Amazon SageMaker hyperpods. See [February's update](/changelog#kubernetes-agent-updates-6) for other recent updates.
...
- **Filter by Commitment ARN:** You can now filter by Commitment ARN on [Financial Commitment Reports](/financial_commitment_reports#create-a-financial-commitment-report) to understand the discount impact of a single Savings Plan or Reserved Instance.
...
- **Kubernetes Efficiency Report export:** You can now export [Kubernetes Efficiency Reports](/kubernetes#export-kubernetes-efficiency-reports) from the console.
...
*See [September's update](/changelog#kubernetes-agent-updates-6) for the most recent Kubernetes agent release.*
...
- **Costs drilldown:** New costs [drilldown options](/cost_reports#drill-down-in-costs-table) are available to add additional targeted filters and grouping criteria to a Cost Report.
...
- **Datadog integration update:** Vantage provides a [visual alert](/connecting_datadog#update-the-datadog-integration) in the integration settings screen when the Datadog integration requires an update.
...
- **Account filter matching:** For cost providers, like AWS, that have human-readable account names, you can now wildcard-match on account names in [Cost Report filters](/cost_reports#filter-cost-reports) (e.g., set a filter, like *Account **contains** dev*).
...
- **Set default dashboard:** You can set the [default dashboard](/dashboards#set-a-default-dashboard) that's displayed for a team or individual users when they log in to Vantage.
...
*See [May's update](/changelog#kubernetes-agent-updates-15) for the most recent Kubernetes agent release.*
...
- **Flow weight on Network Flow Reports:** You can now change the Sankey diagram to be weighted by either costs or bytes on [Network Flow Reports](/network_flow_reports#adjust-flow-weight).
- **Save as new on reports:** On existing reports, like a [Cost Report](/cost_reports#saving-a-cost-report), you can now click **Save as New** at the top to create a new report based on the current report's filters.
...
- **VPC Flow Logs active resources tab:** The **VPC Flow Logs** tab is available on the [Active Resources screen](/active_resources#vpc-flow-logs) for resources that generate network traffic. This tab shows network flows for related resources.
...
- **Kubernetes GPU idle costs:** For each Kubernetes pod, you can now view the [idle and total costs](/kubernetes#kubernetes-gpu-idle-costs) for NVIDIA GPU usage within a Kubernetes cluster.
...
- **Datadog metrics:** Business metrics have been updated with the option to directly ingest [Datadog metrics](/per_unit_costs#import-from-datadog).
...
- **New rightsizing recommendations:** Kubernetes [rightsizing recommendations](/cost_recommendations#kubernetes-rightsizing-recommendations) are now available for rightsizing managed workloads in Kubernetes.
...
- **API service tokens:** Users with owner-level permissions can now create [API service tokens](/vantage_account#create-an-api-token) for authenticating with the Vantage API at the account level, rather than being associated with a specific user.

### Kubernetes Agent Updates

*See [March's update](/changelog#kubernetes-agent-updates-1) for the most recent Kubernetes agent release.*
...
- **Saved filters on dashboards**: You can now add a [saved filter](/dashboards#add-a-saved-filter-to-a-dashboard) to a dashboard to view all Cost Reports on the dashboard with consistent filters applied.
...
- **Custom date filtering**: The date picker on [Cost Reports](/cost_reports#create-a-cost-report) is updated to allow for custom period selections.
...
- **"Other" grouping expanded**: The [**Other** grouping](/cost_reports#starred-by-me) is expanded on Cost Reports so you can display additional costs, beyond what's already displayed, in the table and graph.
...
- **Write API**: The new write [API](/api-reference) automates the management of Cost Reports, saved filters, and dashboards.
...
- **Budget alerts**: You can now create [budget alerts](/budgets#budget-alerts-and-cost-reports) to receive automated notifications when costs exceed user-defined alert thresholds.
...
- **Cost Report exports**: You can now [export Cost Reports](/cost_reports#export-cost-reports) via custom exports.
